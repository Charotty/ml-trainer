#!/usr/bin/env python3
"""
Lasso Regression Model Training for Kidney Displacement Prediction
L1 regularized model with feature selection capabilities
"""
...
class LassoTrainer:
    ...
    def prepare_training_data(self, df):
        ...
        # Scale features (critical for Lasso regularization)
    ...
    def feature_selection(self, X_train, y_train, feature_names):
        ...
        # Method 3: RFE with Lasso
        lasso = Lasso(random_state=42, max_iter=2000)
        rfe = RFE(estimator=lasso, n_features_to_select=min(20, len(feature_names)))
    ...
    def hyperparameter_tuning(self, X_train, y_train):
        """Perform optimized hyperparameter tuning for Lasso"""
        print("\nPerforming optimized hyperparameter tuning for Lasso...")
    ...
    def train_and_evaluate(self, X_train, X_test, y_train, y_test, use_tuning=True):
        ...
        print(f"\nTraining and evaluating optimized Lasso models...")
    ...
    def generate_report(self):
        ...
        print("\n" + "="*80)
        print("OPTIMIZED LASSO REGRESSION MODEL - DETAILED TRAINING REPORT")
        print("="*80)
        ...
        alphas = [r['Alpha'] for r in self.results.values()]
        print(f"Alpha range: {min(alphas):.4f} - {max(alphas):.4f}")
        print(f"Average alpha: {np.mean(alphas):.4f}")
    ...
    def save_results(self, filename='lasso_results.csv'):
        """Save results to CSV"""
        rows = []
        for target_name, metrics in self.results.items():
            row = {
...
def main():
    """Main training pipeline for Lasso regression"""
